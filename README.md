...
底层 Agent Runtime 基于 pi-mono 做了定制维护，并以 `@earendil-works/*` 包的形式接入。本项目并不是直接依赖原版 pi-mono，而是使用适配了 GUI、IM、多工具运行和插件集成需求的 fork。
...
- 底层 Agent Runtime 基于 pi-mono 的定制 fork，当前以 `@earendil-works/*` 包接入，包含包名、构建方式和部分运行时适配。
...
- Agent Runtime：基于 pi-mono 定制的 `@earendil-works/pi-ai`、`@earendil-works/pi-agent-core`、`@earendil-works/pi-coding-agent`
...
## 关于 `@earendil-works/*` Git 依赖

本项目使用 `@earendil-works/pi-ai`、`@earendil-works/pi-agent-core`、`@earendil-works/pi-coding-agent` 作为 Agent runtime。这些包来自对 pi-mono 的定制 fork，包含了为 PiAgent-GUI 适配的包名、构建方式和部分运行时调整。
